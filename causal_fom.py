...
filename_to_save = 'aug_18_200_sep_9'  #G2R16
# df = df.head(10000)
print(df.columns)
print(df.shape)
...
def add_noise_to_duplicates(arr, noise_scale=0.01):
    # Add non guassian noise using uniform distribution
...
def get_bootstrap_data(arr):
    """
    Generate multiple bootstrap samples from the original dataset using random sampling with replacement.
    ...
    # Add uniform noise to the bootstrap sample to avoid duplicate issues
...
def apply_bootstrap_zero_rule(original_B, bootstrapped_B_list, zero_fraction=0.5):
    """
    For each (i, j), if >= zero_fraction of bootstrapped B matrices have bij == 0, set original_B[i, j] = 0.
    """
    B = original_B.copy()
    n_boot = len(bootstrapped_B_list)
    stacked = np.stack(bootstrapped_B_list, axis=0)
    zero_counts = np.sum(stacked == 0, axis=0)
    mask = zero_counts >= (zero_fraction * n_boot)
    B[mask] = 0
    return B


# Utility: Threshold B matrix values

def threshold_b_matrix(B, threshold=0.01):
    """
    Set entries of B with abs(value) < threshold to zero.
    Works for both original and bootstrapped B matrices.
    """
    B = B.copy()
    B[np.abs(B) < threshold] = 0
    return B

def threshold_b_matrices_list(B_list, threshold=0.01):
    return [threshold_b_matrix(B, threshold) for B in B_list]
...
for L in range(Lmin, Lmax + 1, Linc):
    ...
    if bootstrap_exp is True:
        # Threshold the original B matrix before bootstrapping
        B_adjacency_matrix_org = threshold_b_matrix(B_adjacency_matrix_org, threshold=0.01)
        for _ in tqdm(range(ITER)):
            # Generate bootstrap sample with noise using the new functions
            data = get_bootstrap_data(X)
            B_adjacency_matrix, W, nodes_permutation = two_step_CD(data)
            # Threshold the bootstrapped B matrix
            B_adjacency_matrix = threshold_b_matrix(B_adjacency_matrix, threshold=0.01)
            b_matrices_list.append(B_adjacency_matrix)

        # Apply the 50% rule: if 50% of bootstrapped bij are zero, set original bij to zero
        B_adjacency_matrix_org = apply_bootstrap_zero_rule(B_adjacency_matrix_org, b_matrices_list, zero_fraction=0.5)

        # Rest of the bootstrap code remains unchanged
